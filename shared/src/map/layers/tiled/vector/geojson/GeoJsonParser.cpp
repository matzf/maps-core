...
static std::shared_ptr<GeoJsonGeometry> parseFeature(const nlohmann::json &feature, UUIDGenerator &uuidGenerator, StringInterner &stringTable) {
    ...
    if (feature.contains("id") && feature["id"].is_string()) {
        geometry->featureContext = std::make_shared<FeatureContext>(stringTable, geomType, properties, feature["id"].get<std::string>());
    } else {
        geometry->featureContext = std::make_shared<FeatureContext>(stringTable, geomType, properties, uuidGenerator.generateUUID());
    }
    return geometry;
}
...
std::shared_ptr<GeoJson> GeoJsonParser::getGeoJson(const nlohmann::json &geojson, StringInterner &stringTable) {
    ...
    if (type == "Feature") {
        ...
        geometry->featureContext = std::make_shared<FeatureContext>(stringTable, geomType, FeatureContext::mapType(), generator.generateUUID());
        geoJson->geometries.emplace_back(std::move(geometry));
    }
