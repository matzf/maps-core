...
TEST_CASE("GetPropertyValue tests", "[GetPropertyValue]") {
    StringInterner stringTable = ValueKeys::newDerivedStringTable();
    auto key = stringTable.add("key");
    auto featureContext = std::make_shared<FeatureContext>(stringTable, vtzero::GeomType::POINT, FeatureContext::mapType{}, 0);
...
TEST_CASE("CaseValue Test", "[CaseValue]") {
    StringInterner stringTable = ValueKeys::newDerivedStringTable();
    auto key = stringTable.add("key");
    auto featureContext = std::make_shared<FeatureContext>(stringTable, vtzero::GeomType::POINT, FeatureContext::mapType{{key, "value"}}, 0);
    EvaluationContext context = EvaluationContext(0, 0, featureContext, nullptr);
